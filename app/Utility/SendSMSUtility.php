...
class SendSMSUtility
{
    ...
    {
        if(OtpConfiguration::where('type', 'smsgatewayhub')->first()->value == 1) {
            
            $text = rawurlencode($text);
            
            $api_key   = env('SMSGHUB_API_KEY');
            $sender_id = env('SMSGHUB_SENDER_2');
            $to        = str_replace('+', '', $to);
            $ch = curl_init('https://www.smsgatewayhub.com/api/mt/SendSMS?APIKey='.$api_key.'&senderid='.$sender_id.'&channel=2&DCS=0&flashsms=0&number='.$to.'&text='.$text.'&route=1');
