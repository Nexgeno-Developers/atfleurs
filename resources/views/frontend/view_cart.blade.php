                        ...
                        <div class="shadow-sm bg-white p-md-3 p-2 p-lg-4 rounded text-left">
                                                ...
                                                <div class="col-lg col-6 order-4 order-lg-0">
                                                    @if ($cartItem['digital'] != 1 && $product->auction_product == 0)
                                                        <div
                                                            ...
                                                            <input readonly type="number" name="quantity[{{ $cartItem['id'] }}]"
                            ...
                            <div class="row align-items-center shipoping_bottom_sec">
                                ...
                                <div class="col-md-6 col-12 order-md-2 order-1 text-center text-md-right">
                                    @if (Auth::check())
                                        <a href="{{ route('checkout.store_delivery_info2') }}" class="btn btn-success fw-600">
...
@section('modal')
    ...
    {{--/* otp popup  */ --}}
        ...
        <div class="modal" id="otp-modal-popup" tabindex="-1" role="dialog">
          <div class="modal-dialog" role="document">
            <div class="modal-content">
              <div class="modal-header">
                ...
                <form class="form-default" id="otp-login-customer-popup" role="form" action="{{ route('user.login.via.otp.verify') }}" method="POST">
                @csrf
                    <div class="modal-body">
...
@section('script')
    ...
    <script type="text/javascript">
        ...
        function closeOtpModal() {
            $('#otp-modal-popup').modal('hide');
        }
        
        function ajax_form_Submit_popup(){
            if (isPhoneShown) {
                
                $('#login-form-customer-popup').off('submit');
                
                $('#login-form-customer-popup').on('submit', function(e) {
        ...
        $('#login-form-customer-popup').attr('action', "{{ route('user.login.via.otp') }}");

        function toggleEmailPhone(el){
            if(isPhoneShown){
                $('#login-form-customer-popup').on('submit');
                $('.phone-form-group').addClass('d-none');
                $('#password').removeClass('d-none');
                $('#login-form-customer-popup').attr('action', "{{ route('login') }}");
        ...
        $('#otp-login-customer-popup').on('submit', function(e) {
            e.preventDefault(); // Prevent the default form submission
            $.ajax({
                type: 'POST',
                url: $(this).attr('action'),
