...
<script>
  function gtag_report_conversion(url) {
    ...
    gtag('event', 'conversion', {
      'send_to': 'AW-11362608793/kop8CKaxgf0YEJnNjqoq',
...
<div class="modal-body p-4 added-to-cart">
    ...
    <div class="media mb-4">
        ...
        @php
        $categoryIds = json_decode($product->category_id, true); // Decode JSON to an array or use explode for comma-separated
        
        $results = collect();
    
        if (is_array($categoryIds) && !empty($categoryIds)) {
            $results = App\Models\Product::where(function ($query) use ($categoryIds) {
                foreach ($categoryIds as $id) {
                    $query->orWhere('category_id', 'LIKE', "%$id%");
                }
            })
            ->where('id', '!=', $product->id) // Exclude the current product
            ->limit(10) // Set to a reasonable number
            ->get();
        }
    @endphp
    ...
    <div class="text-center">
        ...
        <a onclick="return gtag_report_conversion('{{ route('cart') }}');" href="{{ route('cart') }}" class="btn btn-primary mb-3 mb-sm-0">{{ translate('Proceed to Checkout')}}</a>
    </div>
</div>
