...
<div class="aiz-titlebar text-left mt-2 mb-3">
    <div class="row align-items-center">
        ...
        <div class="col text-right">
            <a href="javascript:void(0);" onclick="product_review_admin()" class="btn btn-circle btn-info"><span>{{ translate('Add Review') }}</span> </a>
        </div>
    </div>
</div>
{{--
        ...
        <table class="table aiz-table mb-0">
            <thead>
                <tr>
                    ...
                    <th data-breakpoints="lg">{{translate('Reviewed By')}}</th>
                ...
                @foreach($reviews as $key => $review)
                    @if ($review->product != null && $review->user != null)
                        <tr>
                            ...
                            @if ($review->is_admin_review == 1)
                                <td><span style="width: 100px;" class="badge badge-info">Admin Review</span></td>
                            @else
                                <td><span style="width: 100px;" class="badge badge-info">User Review</span></td>
                            @endif
                            <td>
...
<div class="modal fade" id="product-review-admin-modal">
    <div class="modal-dialog">
        <div class="modal-content" id="product-review-admin-modal-content">
...
@section('script')
    <script type="text/javascript">
        function product_review_admin() {
            $.post('{{ route('product_review_admin_modal') }}', {
                _token: '{{ @csrf_token() }}'
            }, function(data) {
                $('#product-review-admin-modal-content').html(data);
                $('#product-review-admin-modal').modal('show', {
                    backdrop: 'static'
                });
