                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group row" id="category">
                            ...
                            <div class="col-lg-8">
                                <select multiple class="form-control aiz-selectpicker" name="category_id[]" id="category_id" data-selected="{{ $product->category_id }}" data-live-search="true" required>
                        ...
                        <div class="form-group row d-none">
                            ...
                            <div class="col-lg-8">
                                <input type="text" class="form-control" name="unit" placeholder="{{ translate('Unit (e.g. KG, Pc etc)') }}" value="{{$product->getTranslation('unit', $lang) ?? 'kg'}}" required>
                            </div>
                        </div>
                        <div class="form-group row d-none">
                            <label class="col-md-3 col-from-label">{{translate('Weight')}} <small>({{ translate('In Kg') }})</small></label>
                            <div class="col-md-8">
                                <input type="number" class="form-control" name="weight" value="{{ $product->weight ?? '0.00' }}" step="0.01" placeholder="0.00">
                            </div>
                        </div>
                        <div class="form-group row">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group row gutters-5">
                            ...
                            <div class="col-lg-1">
                                <label class="aiz-switch aiz-switch-success mb-0">
                                    <input value="1" type="checkbox" name="colors_active" id="colors_active" <?php if (count(json_decode($product->colors)) > 0) echo "checked"; ?> >
                                    <span></span>
                                </label>
                            </div>
                        </div>
...
<!--                <div class="card">
            ...
            <div class="col-lg-4">
               ...
               <!--HSN CODE-->
                <div class="card">
                    <div class="card-header">
                        <h5 class="mb-0 h6">{{translate('HSN')}}</h5>
                    </div>
                    <div class="card-body">
                        <label for="hsn_code">
                            {{translate('HSN Code')}}
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-12">
                                <input type="text" class="form-control" value="{{$product->hsn_code}}" name="hsn_code" required>
                            </div>
                        </div>
                    </div>
                </div>  
                ...
                <div class="card">
                    <div class="card-header">
                        <h5 class="mb-0 h6">{{translate('Inclusive GST')}}</h5>
                    </div>
                    <div class="card-body">
                        <label for="gst">
                            {{translate('GST')}}
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-12">
                                <select class="form-control aiz-selectpicker" name="gst" selected>
                                    <option value="0" @if($product->gst == 0) selected @endif>{{translate('0%')}}</option>
                                    <option value="5" @if($product->gst == 5) selected @endif>{{translate('5%')}}</option>
                                    <option value="12" @if($product->gst == 12) selected @endif>{{translate('12%')}}</option>
                                    <option value="18" @if($product->gst == 18) selected @endif>{{translate('18%')}}</option>
                                    <option value="28" @if($product->gst == 28) selected @endif>{{translate('28%')}}</option>
                                </select>
                            </div>
                        </div>
                    </div>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        @php
                            $zipcode = DB::table('zipcode_availibility')->select('id', 'name')->get();
                        @endphp
                        <div class="form-row">
                            <div class="form-group col-md-12">
                                <select multiple class="form-control aiz-selectpicker" name="zipcode_availibility_id[]" required>
                                    <option value="" disabled>Select Shipping Class</option>
                                    @foreach($zipcode as $code)
                                        @php
                                            $selected = false;
                                            if ($product->zipcode_availibility_id && is_string($product->zipcode_availibility_id)) {
                                                $selected = in_array($code->id, json_decode($product->zipcode_availibility_id, true));
                                            }
                                        @endphp
                                        <option value="{{ $code->id }}" @if($selected) selected @endif>{{ $code->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const colorsActiveCheckbox = document.getElementById('colors_active');
        const colorsSelect = document.getElementById('colors');

        function toggleRequiredAttribute() {
            if (colorsActiveCheckbox.checked) {
                colorsSelect.setAttribute('required', 'required');
            } else {
                colorsSelect.removeAttribute('required');
            }
        }
        ...
        colorsActiveCheckbox.addEventListener('change', toggleRequiredAttribute);
    });
</script>
